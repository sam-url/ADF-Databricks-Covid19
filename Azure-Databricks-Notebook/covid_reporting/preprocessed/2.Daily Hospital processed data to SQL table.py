...
delta_table_read_df = spark.read.format("delta").load(f"{preprocessed_adls_folder_path}/ecdc/hospital_admissions_daily")
...
delta_table= delta_table_read_df.select(col('country'),
 ...
 col('country_code_3_digit'),
  col('population'),
   col('reported_date'),
 col('Daily_hospital_occupancy'),
 col('Daily_ICU_occupancy'),
 col('source'))
...
jdbc_table = "covid_reporting.hospital_admissions_daily"
delta_table.write \
    .format("jdbc") \
    .option("url", jdbc_url) \
